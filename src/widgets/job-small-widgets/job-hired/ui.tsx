...
import style from "./jobHired.module.css";
...
import { getNoHiredEmptyState } from "widgets/CandidatesLists/lib/helpers";
...
export const JobHired = ({ columns }: IJobHired) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await JobApi.getHiredCandidates(id, page, pageSize).then(
        (res) => {
          return {
  ...
  return (
    <Card className={style.container}>
      <div className={style.header}>
        <CardHeader title="Hired" image={userListIcon} />
      ...
      <Table
        ...
        emptyState={getNoHiredEmptyState(emptyAnimation)}
      />
    </Card>
  );
};
